...
class SparkContext(
  ...
  def sequenceFile[K, V](path: String,
    ...
    // 从 sequence的format里面读取 key 和 value
   ...
   */
   // allowLocal是否允许将一些简单的计算安排在master节点上
  ...
  def runJob[T, U: ClassManifest](
      ...
      // 给func包一层，添加上执行上下文( task的执行上下文, 可迭代的数据 ) -> 对所有数据执行 func，分为几个partition
